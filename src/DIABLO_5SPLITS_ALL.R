...
for (i in 0:4) {
  ...
  Xtrain_met_name = paste0(c('XTRAIN_METABOLOMICS_NOREPS_SPLIT_'), i, '.csv')
  Xtrain_met = read.csv(Xtrain_met_name, header = TRUE, sep = ",", row.names=1)
  dim(Xtrain_met)
  
  Xtest_met_name = paste0(c('XTEST_METABOLOMICS_NOREPS_SPLIT_'), i, '.csv')
  ...
  ytest_name = paste0(c('yTEST_ALL_GENES_NOREPS_SPLIT_'), i, '.csv')
